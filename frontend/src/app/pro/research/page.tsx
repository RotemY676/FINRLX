 ...
 * workspace `/pro/research/[ticker]` and to the existing `/pro/backtests` and
 * `/pro/universe` surfaces (the latter remains the coverage / readiness
...
export default function ResearchHubPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTickerError(null);
    router.push(`/pro/research/${normalized}`);
  };
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {universes.map((u) => (
              <li key={u.universe_id}>
                <Link
                  href={`/pro/universe?id=${encodeURIComponent(u.universe_id)}`}
