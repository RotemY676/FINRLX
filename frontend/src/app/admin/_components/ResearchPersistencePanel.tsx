  ...
  return (
    <GlassCard className="mt-4">
      ...
      {status && (
        <>
          {/* Summary strip */}
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3 text-center mb-3">
            <div>
            ...
            <div>
              ...
              <p className="text-[10px] text-ink-4">volume env detected</p>
            </div>
            <div>
              <p className={`text-[11px] font-medium ${status.storage_root_uses_persistent_volume ? "text-pos" : status.is_persistent_volume_configured ? "text-breach" : "text-ink-3"}`}>{status.storage_root_uses_persistent_volume ? "Yes" : "No"}</p>
              <p className="text-[10px] text-ink-4">storage uses volume</p>
            </div>
            <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <span key={key} className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-medium ${
                      (status as unknown as Record<string, unknown>)[key] ? "bg-pos/10 text-pos" : "bg-breach/10 text-breach"
                    }`}>{label}</span>
                  ))}
                </div>
              </div>
