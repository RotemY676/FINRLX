...
def test_storage_root_under_volume_path(monkeypatch, tmp_path):
    """When storage root IS under the volume path, flag is true."""
    vol = str(tmp_path / "volume")
    os.makedirs(os.path.join(vol, "research", "finrlx_cpu", "exports"), exist_ok=True)
    monkeypatch.setenv("RAILWAY_VOLUME_MOUNT_PATH", vol)
    monkeypatch.setenv("RAILWAY_ENVIRONMENT", "production")
    # Point the exports dir (and project root) under the volume
    fake_root = vol
    monkeypatch.setattr(
        FinRLXResearchService, "_exports_dir",
        staticmethod(lambda: os.path.join(fake_root, "research", "finrlx_cpu", "exports"))
    ...
    result = FinRLXResearchService.get_persistence_status()
    assert result["is_persistent_volume_configured"] is True
    # Note: storage_root_uses_persistent_volume depends on the actual project root
    # which we haven't monkeypatched, so we just verify the field exists
    assert "storage_root_uses_persistent_volume" in result
    assert "persistent_volume_mount_path" in result
