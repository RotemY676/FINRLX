...
- **🔴 LOOK-AHEAD LEAK CAUGHT AND FIXED:** the first artifact scored PPO/A2C at **Sharpe 4+**, crushing every real strategy. Cause: the observation carried `row[idx].realized_return` — the exact forward return `step()` rewards on — so the agent saw its own answer. Deleted that artifact, redesigned `_get_obs` to contain only information known *before* the return is realised (engine_score, the prior row's return, trailing vol, prev action), and pinned it with a regression test. Retrained: **PPO 1.60 (per-split [2.97, −0.47, 2.28]), A2C 2.23 ([2.97, 1.16, 2.54])** — realistic, in the same band as the real strategies, with a genuine losing split. **Had I committed the leaked artifact, the dashboard would have shown RL dominating on fiction.**
- **Real research stack installed** (CPU torch 2.13 + SB3 2.9, CPU index on its own line — the flaw the audit flagged). Artifact committed: `research/artifacts/finrl_ensemble/AAPL.json`, splits `[47,63][63,79][79,95]` matching production's current AAPL protocol.
- **Verified merge:** `build_dossier("AAPL")` now returns `rl: artifact_merged`, 10 candidates, and **A2C wins at score 1.69** over Buy & Hold (0.78) under the re-deflated penalty. The RL leg is real, not queued, for AAPL.
- **🟡 Honest limitation:** the committed artifact is valid only until the next weekly rebalance shifts the walk-forward windows, after which the split-protocol check correctly rejects it and the RL lane returns to "queued". Durable RL needs the E7 worker on a schedule; one artifact proves the path end-to-end.
