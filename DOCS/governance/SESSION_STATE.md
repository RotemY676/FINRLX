...
- **⬜ PENDING VERIFICATION — push-triggered auto-deploy.** The green deploys so far were triggered by the `serviceConnect` mutation, not by a GitHub webhook. Cross-account webhook delivery depends on the Railway GitHub App being authorized on the `RotemY676` account. Being verified now against the `8f8a2f2` push; if no deploy appears, install/authorize the Railway GitHub App on `RotemY676` (browser step, user-only).
