...
### Phase 8N.2A targeted (14 tests)
```
14 passed in 0.84s
```

### Full Phase 8 regression (300 tests)
```
300 passed in 236.06s
```
...
## Fix1 Changes (8N.2A-fix1)

- Candidate building is now truly read-only: uses `_load_registry_file_read_only()`
  instead of operational `load_*_registry()` methods that create empty files
- Recursive sanitization via `_sanitize_mirror_payload()` applies to
  metadata_summary_json, warnings_json, limitations_json at all nesting depths
- Secret patterns now catch `bearer <token>` and `postgres://` URLs
- Tests use strict AND-style absence assertions (no `or "REDACTED"` escape hatch)
- DB-level sanitization test proves stored JSON columns have no raw secrets
- Read-only test proves missing files are not created during candidate building
- All-four-kind test creates real items in all 4 registries and asserts coverage
