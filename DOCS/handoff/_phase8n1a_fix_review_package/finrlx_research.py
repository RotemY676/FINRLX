...
class FinRLXResearchService:
    ...
    def get_persistence_status() -> dict:
        ...
        if railway_volume:
            ...
            try:
                real_storage = os.path.realpath(abs_storage_root).replace("\\", "/")
                real_volume = os.path.realpath(railway_volume).replace("\\", "/")
                storage_root_uses_volume = real_storage.startswith(real_volume)
            except Exception:
                storage_root_uses_volume = False
