...
def test_storage_root_uses_volume_integration(monkeypatch):
    """get_persistence_status returns storage_root_uses_persistent_volume=True
    when RAILWAY_VOLUME_MOUNT_PATH is set to an ancestor of the real storage root."""
    # Compute the real project root the same way the service does:
    # finrlx_research.py is at backend/app/services/finrlx_research.py
    import app.services.finrlx_research as mod
    svc_file = os.path.abspath(mod.__file__)
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(svc_file))))
    real_storage = os.path.realpath(os.path.join(project_root, "research", "finrlx_cpu"))
    # Set RAILWAY_VOLUME_MOUNT_PATH to the project root (an ancestor of storage).
    monkeypatch.setenv("RAILWAY_VOLUME_MOUNT_PATH", project_root)
    monkeypatch.setenv("RAILWAY_ENVIRONMENT", "production")
    result = FinRLXResearchService.get_persistence_status()
    assert result["is_persistent_volume_configured"] is True
    assert result["storage_root_uses_persistent_volume"] is True, (
        f"Expected True: storage={real_storage} should be under volume={project_root}"
    )
