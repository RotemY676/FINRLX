...
| **US-P0-06** Zero-fiction audit | **Partially implemented (scan added)** | Adapter classifies demo/synthetic/unknown sources and fails them closed. **Repo-wide static scan now exists**: `app/core/fiction_policy.py` AST-scans serving paths (`app/api`,`app/services`) for fabrication primitives (`random`/`np.random`) + fiction-admitting TODOs, with a reviewed shrink-only baseline (`KNOWN_FICTION_SITES`, 6 sites) + one-way ratchet (`test_p0_fiction_scan.py`: no new site, no stale entry). Scan surfaced the beta synthetic market-data generators (`ingest.py` `_generate_bars`/`_generate_news`) as labeled-synthetic sites. **Linkage enforced (inc 2):** `_classify_source` is now an allowlist (`_REAL_SOURCE_TOKENS={yfinance,chain}`) — the beta "local"/"local_deterministic" source and any unknown/missing provenance are classified `is_synthetic` and failed closed (was fail-open: a nonempty unknown label silently passed as real). Test: `test_p0_synthetic_source_failclosed.py`. **Demo endpoints labeled (inc 3):** `GET /regime`, `POST /scenario/simulate`, `GET /scenario/baseline` now carry a standardized `DEMO_DATA:` label in `meta.warnings` via `make_meta(is_demo=True)` (`test_p0_demo_endpoints_labeled.py`). Full demo-flag gating / real models = later product decision. |
| **US-P0-07** Freshness suppression | **Partially implemented (envelope wired)** | `price_freshness` reused; stale/degraded/unavailable data blocks packet eligibility with reason codes. **Envelope freshness now populated (inc 1):** `meta.freshness` (FreshnessState) was defined but never set — every response read as implicitly fresh. Added reusable `app/services/freshness_state.freshness_state_from_latest` (reuses D6 `classify_lag`+trading calendar) and `make_meta(freshness=...)`; wired into `/pricechart` (declares the age of what it served; no-data → stale, never silently fresh). Test: `test_p0_freshness_envelope.py`. **Follow-up:** wire the same into dossier/analysis/desk/recommendations surfaces (audit list in STATUS). |
| **US-P0-08** Readiness endpoint | **Implemented (data/fx/provider/jobs)** | Admin-only `GET /api/v1/ops/readiness` (`readiness.py` service + schema) aggregates market-data freshness, FX freshness, provider readiness, and job-run health into one overall verdict + affected scope, fail-closed (unevaluable component → `unavailable`). Tests: `test_p0_readiness.py`. Model (shadow-only) + alert readiness are a follow-up (schema already supports them). |
